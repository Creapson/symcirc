"""
Pole/zero node: finds the poles and zeros of a transfer function H(s) and
plots them on the s-plane.
...
class PoleZeroNode(Node):
    ...
    def build(self):
        ...
        with self.add_static_attr():
            ...
            dpg.add_text("Source: connect a TransferFunction node.",
                         tag=self.source_info_tag, color=[150, 150, 150], wrap=340)
            dpg.add_text("The output node and the symbolic/numeric mode are taken "
                         "from the connected TransferFunction node.", color=[150, 150, 150],
                         wrap=340)
            dpg.add_separator()
    ...
    # ------------------------------------------------------------------
    # Numeric path: descriptor (DAE) state-space form + polynomial
    # eigenvalue problem.
    # ------------------------------------------------------------------

    def _extract_poly_coeffs(self, A_mat, s):
        """
        Extract the polynomial coefficient matrices of A(s) in s:
        ...
        # Fast path: if A(s) is affine (A_0 + s*A_1), verify with one
        ...
        if non_poly_found:
            # An entry is rational in s: clear by the common denominator;
            # the denominator roots are dropped later as spurious.
            print("Warning: rational (fractional) entries detected in A(s); "
                  "clearing by the common denominator. The denominator roots "
                  "are marked spurious and removed from the results.", flush=True)
            for i in range(n):
                for j in range(m):
    ...
    def _compute_pencil_scaling(self, A_coeffs, k):
        """
        Scale factor for s = gamma * s_hat. Chosen so the constant term A_0
    ...
    def _equilibrate_pencil(self, A, B, iterations=2):
        """
        Balance the pencil (A, B) by two-sided diagonal scaling: D*A*E,
    ...
    def _build_rosenbrock_coeffs(self, A_coeffs, B_coeffs, C_out, n):
        """
        Build the generalized Rosenbrock system-matrix polynomial
    ...
    def _solve_polynomial_eigenproblem(self, A_coeffs, n, k, gamma, abs_mag_limit=1e18):
        """
        For the degree-k matrix polynomial P(s) = sum_i s^i * A_coeffs[i],
    ...
    def _filter_spurious_roots(self, roots, denom_list, s, rel_tol=1e-6):
        """
        Remove the common-denominator roots introduced by the rational-entry
        clearing step. Takes a LIST of denominators because A(s) and z(s) may
        each be rational. If every denominator is 1 (both pure polynomials -
        the normal case) it is a no-op.
        """
        spurious = []
    ...
    def _resolve_output_node(self, mna_data):
        """
        The output node of the transfer function. POLES are INDEPENDENT of
        ...
        try:
            ...
            print(f"Could not read the unknowns: {e}", flush=True)
            return None
        ...
        print(f"Could not get the output node from the TransferFunction node; "
              f"temporarily using '{chosen}'.", flush=True)
        print(f"Available nodes: {names}", flush=True)
        print("Select the output node in the TransferFunction node and press Calculate.",
              flush=True)
        return chosen

    def _calculate_robust_poles_zeros(self, mna_data, unknown_variable: str):
        """
        Numeric pole/zero solve (descriptor / DAE formulation).
        ...
        try:
            ...
            print(f"Detected A(s) polynomial degree: k={k}"
                  + (" (affine / first-order descriptor DAE: A(s)=G+s*C_dyn, no truncation)"
                     if k <= 1 else
                     " (higher-order dynamics detected - full PEP solve applied)")
                  + f" | cond(G)={cond_g:.3e} | ||C_dyn||/||G||={dyn_range:.3e}",
                  flush=True)

            # Extract z as a full polynomial too (a Norton transform can
            # make z depend on s).
            B_coeffs, k_z, denom_lcm_z = self._extract_poly_coeffs(z_sym, s)
            if k_z > 0:
                print(f"NOTE: the source vector z(s) DEPENDS on s (degree {k_z}); "
                      f"using it as a full polynomial.", flush=True)

            # Scale C_out to the size of G: does NOT change the roots, it
            ...
            # sort by (|z|, Re, Im): |z| alone cannot separate conjugates.
            poles_num = sorted(poles_num, key=lambda v: (abs(v), v.real, v.imag))

            # ZEROS
            if k == 1 and k_z <= 1:
                # Replace the output column with z; the finite eigenvalues
                ...
                # Higher order / s-dependent source: Rosenbrock system
            ...
            print(f"Numeric PEP solve error: {e}", flush=True)
            zeros_num = []
            poles_num = []
    ...
    # ------------------------------------------------------------------
    # Symbolic path: H(s) = det(A_out(s)) / det(A(s)) as an exact rational.
    # ------------------------------------------------------------------
    def _to_exact(self, v):
        """
        float -> exact rational. repr(float) is the shortest round-tripping
        decimal, so Rational(repr(x)) captures the intended value
        (e.g. a 1e-9 capacitance) exactly.
        """
        f = float(v)
        if f != f or f in (float("inf"), float("-inf")):
            raise ValueError(f"value is not finite: {v!r}")
        try:
    ...
    def _roots_of_exact_poly(self, poly, s):
        """
        Roots of an exact-coefficient polynomial. First the trailing zero
    ...
    def _det_poly(self, M, s):
        """
        Return det(M(s)) as an exact POLYNOMIAL in s (the caller clears any
    ...
    def _clear_s_denoms(A, A_out, s):
        """
        Inductors are stamped as 1/(s*L) in the MNA, so A(s) is RATIONAL in
    ...
    def _calculate_symbolic_poles_zeros(self, mna_data, unknown_variable: str):
        """
        Symbolic (closed-form) pole/zero solve.
        ...
        if unknown_variable not in x_syms:
            raise ValueError(f"'{unknown_variable}' is not among the unknowns.")
        ...
        n = A.shape[0]
        if n > 24:
            print(f"WARNING: {n}x{n} symbolic determinant - this may take a while.",
                  flush=True)

        # A must depend only on s; if an unresolved element symbol remains
        # the symbolic path is meaningless -> fall back to numeric.
        extra = A.free_symbols - {s}
        if extra:
            raise ValueError(f"Unresolved symbol(s) in the MNA matrix: "
                             f"{', '.join(map(str, extra))}")

        A_out = A.copy()
        A_out[:, idx_out] = z

        # With inductors A(s) is rational in s (1/(s*L)); make both matrices
        ...
        if D_expr == 0 or D_expr.has(sp.nan, sp.zoo, sp.oo) \
            ...
            raise ValueError("symbolic determinant singular / undefined "
                             "(D=0 or nan) - falling back to the numeric path")

        # Cancel common (s-dependent) factors. The determinants can be
        ...
        print(f"Symbolic: numerator degree {z_poly.degree()}, "
              f"denominator degree {p_poly.degree()}.", flush=True)
    ...
    def _factored_tf_string(self, zeros, poles, z_poly, p_poly, s) -> str:
        """
        Render H(s) in a readable factored form:
    ...
    def _update_source_info(self):
        if not (self.source_info_tag and dpg.does_item_exist(self.source_info_tag)):
            return
        if self.src_mode:
            label = {"numeric": "TF Numeric (QZ)",
                     "symbolic": "TF Symbolic (det N(s)/D(s))"}.get(
                        self.src_mode, self.src_mode)
            node = self.src_output_node or "(not selected in the TF node)"
            dpg.set_value(self.source_info_tag,
                          f"Source: {label}\nOutput node: {node}")
        else:
            dpg.set_value(self.source_info_tag,
                          "Source: connect a TransferFunction node "
                          "(or an MNA directly).")
    ...
    def onlink_callback(self):
        try:
            ...
            print(f"Could not read the source context: {e}", flush=True)
        super().onlink_callback()

    def calculate_callback(self, sender, app_data, user_data=None):
        raw = self.get_input_pin_value("mna_pin")

        print("\n--- POLE/ZERO CALCULATION TRIGGERED ---", flush=True)
        print(f"First data type from the wire: {type(raw)}", flush=True)
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            dpg.set_value(self.text_zeros_tag, "Could not build MNA!")
            dpg.set_value(self.text_poles_tag, str(e))
            return

        if not mna_data or not hasattr(mna_data, 'A') or not hasattr(mna_data, 'z'):
            err_obj = type(mna_data).__name__ if mna_data else "None (no data)"
            print(f"Error: expected attributes not found! Current object: {err_obj}", flush=True)
            dpg.set_value(self.text_zeros_tag, f"Error: Received {err_obj}")
        ...
        try:
            ...
            if not target_node:
                dpg.set_value(self.text_zeros_tag, "Could not select an output node.")
                dpg.set_value(self.text_poles_tag, "Could not read the MNA unknowns.")
                return
            symbolic_mode = (self.src_mode == "symbolic")
            print(f"Computing for the target node ({target_node}) "
                  f"[{'symbolic' if symbolic_mode else 'numeric QZ'} path]...",
                  flush=True)

            pz_results = None
            if symbolic_mode:
                print("Came from TF Symbolic; solving H(s) as an exact rational "
                      "(may be slow for large circuits)...", flush=True)
                try:
                    ...
                    import traceback
                    traceback.print_exc()
                    print(f"Symbolic solve failed ({e}); falling back to the "
                          f"numeric QZ path.", flush=True)
            ...
            print("Plot completed successfully and axes fitted!", flush=True)
        except Exception as e:
            import traceback
            traceback.print_exc()
