...
# sp.pprint(mna.A)
# sp.pprint(mna.z)
# print("ValueDict",mna.value_dict)
...
idx_out = x_syms.index(sp.symbols("V_2"))
...
approx = ap.approximate(sp.symbols('V_1'), sp.symbols('V_2'), ((1e5,0.50),(1e6, 0.5), (1e7, 0.5)), "tbt",0.6, "max", 1)
...
subs_dict = {sym: 1 for sym in approx_num.free_symbols if sym != sp.symbols("s")}

approx_num = approx_num.subs(subs_dict)

print("\n\n\n\n\n\n")
print(approx_num)

print("\n\n\n\n\n\n")
print(mna.value_dict)
...
ax_mag.semilogx(f, abs(approx_eval), "r--", label="Approximiert")
...
ax_phase.semilogx(f, np.angle(approx_eval, deg=True), "r--", label="Approximiert")
